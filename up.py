...
class Tree:
    ...
    def sources(self):
        """Every .py file in the repository, pending edits included."""
...
def assert_no_key_file_was_missed(tree: Tree) -> None:
    ...
    if found != listed:
        raise Halt(
            ...
            f"  carrying keys but not listed: {sorted(found - listed) or 'none'}\n"
            f"  listed but carrying none:     {sorted(listed - found) or 'none'}"
        )
    print(f"  file list agrees with a repo sweep ({len(listed)} files)")
...
def main() -> int:
    ...
    print("\nThen, once both are green:  python scripts/up.py --finish")
    return 0
